...
export default {
  ...
  methods: {
    add () {
      ...
      publicFunc.ajaxPost({
        url: process.env.API_ROOT + 'admin/role/edit',
        data: this.form,
        success: res => {
          console.log(res)
          if (res.body.code === 1) {
  ...
  created: function () {
   publicFunc.ajaxPost({
    ...
    publicFunc.ajaxPost({
      url: process.env.API_ROOT + 'admin/role/info',
      data: {id: id},
      success: res => {
        this.form = res.body.data
        delete (this.form.create_time)
        delete (this.form.update_time)
        let checkkey = this.form.rules
        checkkey = checkkey.split(',')
